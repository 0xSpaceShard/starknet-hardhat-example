...
describe("Amarna test", function () {
     ...
     */
    it("should generate out.sarif file", async () => {
        // Cleanup old sarif file if exists
        fs.existsSync("./out.sarif") && fs.unlinkSync("./out.sarif");

        console.log("Running amarna container");
        await hardhat.amarnaDocker.run({});
        ...
        return new Promise((res) => {
            ...
            const isSarifFileGenerated = () => {
                ...
                expect(result).to.equal("sarif.out");
                res();
            };
